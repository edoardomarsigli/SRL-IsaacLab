...
def run_simulator(sim: sim_utils.SimulationContext, entities: dict[str, Articulation], origins: torch.Tensor):
	...
	while simulation_app.is_running():
		...
		for robot in entities.values():
			# Camminata rudimentale in avanti: sinusoidale sugli arti per simulare movimento
			phase = torch.tensor(sim_time * 2.0)  # la frequenza del passo
			joint_pos_target = robot.data.default_joint_pos.clone()
	
			# Applica una sinusoide su ogni articolazione (semplificata, per esempio dimostrativo)
			if robot.data.joint_pos.shape[1] >= 4:
				joint_pos_target[:, 0] += 0.25 * torch.sin(phase)  # anteriore sinistro
				joint_pos_target[:, 1] += 0.25 * torch.sin(phase + torch.pi)  # anteriore destro
				joint_pos_target[:, 2] += 0.25 * torch.sin(phase + torch.pi)  # posteriore sinistro
				joint_pos_target[:, 3] += 0.25 * torch.sin(phase)  # posteriore destro

			# Applica il target ai giunti

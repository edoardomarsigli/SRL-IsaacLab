...
# Pre-defined configs
##
from isaaclab_tasks.manager_based.moonshot.descriptions.config.moonbot_cfgs import CARTER_CFG  # isort: skip
from isaaclab_tasks.manager_based.moonshot.velocity.terrain.rough import ROUGH_HILLY_TERRAINS_CFG
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformBodyVelocityCommandCfg(
        asset_name="robot",
        body_name="com_offset",
...
class RewardsCfg:
    ...
    # )
    track_lin_vel_xy_exp = RewTerm(
        func=mdp.track_lin_vel_xy_exp_vehicle, weight=1.0, params={"command_name": "base_velocity",
                                                                   "body_name": "com_offset",
                                                                   "std": math.sqrt(0.25)}
    )
    track_ang_vel_z_exp = RewTerm(
        func=mdp.track_ang_vel_z_exp_vehicle, weight=0.5, params={"command_name": "base_velocity",
                                                                  "body_name": "com_offset",
                                                                  "std": math.sqrt(0.25)}
    )
    # -- penalties
